      ...
      <div className="grid h-[calc(100vh-11rem)] grid-cols-1 gap-6 lg:grid-cols-[340px_1fr]">
        <Card className="flex flex-col overflow-hidden">
          <div className="border-b border-gray-100 p-3">
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search discussions..." className="w-full rounded-lg border border-gray-200 py-2 pl-8 pr-2 text-sm focus:border-brand-blue focus:outline-none" />
              </div>
              {canCreate && (
                <button onClick={() => setNewOpen(true)} title="New discussion" className="rounded-lg bg-brand-blue p-2 text-white hover:bg-brand-blue-dark">
          ...
          <div className="flex-1 divide-y divide-gray-50 overflow-y-auto">
            {visible.length === 0 && <p className="p-6 text-center text-sm text-gray-400">{threads.length ? "No matching threads." : role === "business" ? "Start a discussion with your auditor." : "No client discussions yet."}</p>}
            {visible.map((t) => (
        ...
        <Card className="flex flex-col overflow-hidden">
          {!active ? (
            <div className="flex flex-1 flex-col items-center justify-center text-gray-400">
              ...
              <p className="mt-2 text-sm">Select a discussion</p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={send} className="flex items-end gap-2 border-t border-gray-100 p-3">
                <textarea
                  ...
                  placeholder={`Reply to ${role === "business" ? active.auditorName : active.companyName}...`}
          ...
          <Card className="w-full max-w-lg p-0">
            ...
            <form onSubmit={createThread} className="space-y-4 p-6">
              ...
              <Field label="Topic" required>
                <Input value={draft.topic} onChange={(e) => setDraft({ ...draft, topic: e.target.value })} required placeholder="e.g. Treatment of entertainment expenses" />
              </Field>
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="secondary" onClick={() => setNewOpen(false)} disabled={busy}>Cancel</Button>
                <Button type="submit" disabled={busy}>{busy ? "Posting..." : "Start Discussion"}</Button>
              </div>
            </form>
          </Card>
        </div>
