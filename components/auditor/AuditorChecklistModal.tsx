...
export default function AuditorChecklistModal({ engagementId, companyName, existing, onClose, onPublished }: { engagementId: string; companyName: string; existing: ChecklistItem[]; onClose: () => void; onPublished: () => void }) {
  ...
  async function publish() {
    ...
    try {
      ...
      toast.success(`Checklist published to ${companyName}.`);
      onPublished();
    } catch (e) {
      ...
      <Card className="flex max-h-[90vh] w-full max-w-2xl flex-col p-0">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-gray-900">Document Checklist — {companyName}</h2>
              <p className="text-xs text-gray-500">Load a preset, then add or remove items. Items already provided keep their documents.</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 && <p className="py-6 text-center text-sm text-gray-400">Pick a preset or add custom requirements.</p>}
          <ul className="space-y-2">
            {items.map((it, idx) => (
              <li key={idx} className="flex items-start gap-3 rounded-lg border border-gray-100 p-3">
                ...
                <button onClick={() => setItems(items.map((x, i) => (i === idx ? { ...x, required: !x.required } : x)))} className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${it.required ? "bg-red-50 text-red-700" : "bg-gray-100 text-gray-600"}`}>{it.required ? "Required" : "Optional"}</button>
          ...
          <form
            onSubmit={(e) => {
              ...
              setItems([...items, { name: custom.name.trim(), category: custom.category.trim() || "Custom", description: "", required: true }]);
              setCustom({ name: "", category: "" });
            }}
            className="mt-4 flex gap-2"
          >
            <Input value={custom.name} onChange={(e) => setCustom({ ...custom, name: e.target.value })} placeholder="Custom requirement, e.g. Directors' loan schedule" />
            <Input value={custom.category} onChange={(e) => setCustom({ ...custom, category: e.target.value })} placeholder="Category" className="w-40" />
            <Button type="submit" variant="secondary" icon={<Plus className="h-4 w-4" />}>Add</Button>
          </form>
        </div>

        <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
          <p className="text-xs text-gray-500">{items.length} item(s) · {items.filter((i) => i.required).length} required</p>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={onClose} disabled={busy}>Cancel</Button>
            <Button onClick={publish} disabled={busy || items.length === 0}>{busy ? "Publishing..." : "Save & Publish to Client"}</Button>
          </div>
        </div>
      </Card>
    </div>
